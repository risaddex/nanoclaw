...
export function getMessageContentById(id: string, chatJid: string): string | undefined {
  const row = db.prepare(`SELECT content FROM messages WHERE id = ? AND chat_jid = ?`).get(id, chatJid) as
    | { content: string }
    | undefined;
  return row?.content;
}
